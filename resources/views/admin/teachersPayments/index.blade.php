...
@section('title', 'Teachers Payments — List')
@section('content')
    @php
        $months = [
            1 => 'January',
    ...
    <div class="card">
        <div class="card-header">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                ...
                <form method="GET" action="{{ route('admin.teachers-payments.index') }}"
                    ...
                    @if (request('month') || request('year') || request('teacher_id') || request('batch_id'))
                        <a href="{{ route('admin.teachers-payments.index') }}"
                            class="px-3 py-1.5 text-red-600 hover:underline text-sm">
                            Clear
                        </a>
                    @endif
                        ...
                        @foreach ($teachersPayments as $key => $teachersPayment)
                            ...
                            <tr data-entry-id="{{ $teachersPayment->id }}">
                                ...
                                <td>
                                    @if ($status === 'paid')
                                        <span
                                            class="px-2 py-1 rounded-full text-xs font-semibold bg-green-100 text-green-800">Paid</span>
                                    @elseif($status === 'partial')
                                        <span
                                            class="px-2 py-1 rounded-full text-xs font-semibold bg-yellow-100 text-yellow-800">Partial</span>
                                    @else
                                        <span
                                            class="px-2 py-1 rounded-full text-xs font-semibold bg-red-100 text-red-800">Due</span>
                                    @endif
                                </td>
                                <td>
                                    @can('teachers_payment_show')
                                        <a class="btn btn-xs btn-primary"
                                            href="{{ route('admin.teachers-payments.show', $teachersPayment->id) }}">
                                            {{ trans('global.view') }}
                                        </a>
                                    @endcan
                                    ...
                                    @can('teachers_payment_edit')
                                        <a class="btn btn-xs btn-info"
                                            href="{{ route('admin.teachers-payments.edit', $teachersPayment->id) }}">
                                            {{ trans('global.edit') }}
                                        </a>
                                    @endcan

                                    @can('teachers_payment_delete')
                                        <form action="{{ route('admin.teachers-payments.destroy', $teachersPayment->id) }}"
...
@section('scripts')
    ...
    <script>
        $(function() {
            ...
            let table = $('.datatable-TeachersPayment:not(.ajaxTable)').DataTable({
                buttons: dtButtons
            })
