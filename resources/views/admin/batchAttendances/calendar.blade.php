...
@section('title', 'Batch Attendance — Calendar')
@section('content')
    <style>
        .calendar-wrap {
            padding: 18px;
    ...
    <div class="calendar-wrap">
        <div class="calendar-hero">
            <div style="display: flex; justify-content: space-between; align-items: flex-start;">
                ...
                <a href="{{ route('admin.batch-attendances.index') }}" class="btn btn-info"
        ...
        <form method="GET" action="{{ route('admin.batch-attendances.calendar') }}" class="filter-bar">
            ...
            <div class="filter-group">
                ...
                <div class="month-nav-btns">
                    @php
                        $prevMonth = \Carbon\Carbon::createFromDate($selectedYear, $selectedMonth, 1)->subMonth();
                        $nextMonth = \Carbon\Carbon::createFromDate($selectedYear, $selectedMonth, 1)->addMonth();
                    @endphp
                    <a href="{{ route('admin.batch-attendances.calendar', ['batch_id' => $selectedBatchId, 'month' => $prevMonth->format('Y-m')]) }}"
                        class="month-nav-btn">&lt; Prev</a>
                    <input type="month" name="month"
                        value="{{ $selectedYear }}-{{ str_pad($selectedMonth, 2, '0', STR_PAD_LEFT) }}"
                        onchange="this.form.submit()">
                    <a href="{{ route('admin.batch-attendances.calendar', ['batch_id' => $selectedBatchId, 'month' => $nextMonth->format('Y-m')]) }}"
                        class="month-nav-btn">Next &gt;</a>
                </div>
            </div>
        </form>
                    ...
                    @foreach ($calendarData as $day)
                        ...
                        <div class="calendar-cell {{ !$day['has_class'] ? 'no-class' : '' }}">
                            ...
                            @if ($day['has_class'])
                                <div class="stats-row">
                                    @if ($day['present'] > 0)
                                        <span class="stat-pill present">{{ $day['present'] }}P</span>
                                    @endif
                                    @if ($day['absent'] > 0)
                                        <span class="stat-pill absent">{{ $day['absent'] }}A</span>
                                    @endif
                                    @if ($day['late'] > 0)
                                        <span class="stat-pill late">{{ $day['late'] }}L</span>
                                    @endif
                                </div>
