            ...
            <div class="flex items-start justify-between gap-4">
                ...
                @if($teacher->user)
                    <a href="{{ route('admin.users.edit', $teacher->user_id) }}"
                        class="shrink-0 px-4 py-2 rounded-lg text-sm font-medium text-white bg-primary hover:bg-primary-hover shadow-lg shadow-primary/30 transition-all flex items-center gap-2">
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <input
                                    ...
                                    value="{{ old('emloyee_code', $teacher->user->user_name ?? generateUserName()) }}" />
                            </div>
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <input
                                    ...
                                    value="{{ old('joining_date', $teacher->joining_date ? \Carbon\Carbon::parse($teacher->joining_date)->format('d-M-Y') : '') }}" />
...
@section('scripts')
    <script>
        $(document).ready(function () {
            ...
            // Handle pre-existing images in the form if needed (already handled by hidden inputs added by script below)
            @if(isset($teacher) && $teacher->profile_img)
                const initialHiddenInput = document.createElement('input');
                initialHiddenInput.type = 'hidden';
                initialHiddenInput.name = 'profile_img';
                initialHiddenInput.value = '{{ $teacher->profile_img->file_name }}';
                teacherForm.appendChild(initialHiddenInput);
            @endif
